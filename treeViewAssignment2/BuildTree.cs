...
namespace treeViewAssignment2
{
    public class BuildTree
    {
        ...
        public Node fakeEmployeeTree()
        {
            Node e_root_node = new Node();
            e_root_node.create_employee("Root", null, 0, "Root", "", false, true, root_node);
            Tree ETtree = new Tree(e_root_node);

            Node manager_node = new Node();
            manager_node.create_employee("Capybara","Root",20000, "Manager", "", false, true, root_node.childnodes[0]);
            ETtree.addNode(e_root_node, manager_node);

            Node projmanager_node = new Node();
            projmanager_node.create_employee("Hedgehog","Manager",18000,"Project Manager","",false,true, root_node.childnodes[0].childnodes[0]);
            ETtree.addNode(manager_node, projmanager_node);

            Node projleader_node = new Node();
            projleader_node.create_employee("Duck","Project Manager", 16000, "Project Leader", "", false, true, root_node.childnodes[0].childnodes[0].childnodes[0]);
            ETtree.addNode(projmanager_node, projleader_node);

            Node BED_node = new Node();
            BED_node.create_employee("A", "Project Leader", 12000, "Backend Developer", "", false, true, root_node.childnodes[0].childnodes[0].childnodes[0].childnodes[0]);
            ETtree.addNode(projleader_node, BED_node);

            Node FED_node = new Node();
            FED_node.create_employee("B", "Project Leader", 12000, "Frontend Developer", "", false, true, root_node.childnodes[0].childnodes[0].childnodes[0].childnodes[1]);
            ETtree.addNode(projleader_node, FED_node);

            Node DB_node = new Node();
            DB_node.create_employee("C", "Project Leader", 12000, "Database Engineer", "", false, true, root_node.childnodes[0].childnodes[0].childnodes[0].childnodes[2]);
            ETtree.addNode(projleader_node, DB_node);

            Node sysanalyst_node = new Node();
            sysanalyst_node.create_employee("D", "Project Leader", 12000, "System Analyst", "", false, true, root_node.childnodes[0].childnodes[0].childnodes[0].childnodes[3]);
            ETtree.addNode(projleader_node, sysanalyst_node);
        ...
        public void for_tree_view(TreeView treeview1, Form1 form1)
        {
            ...
            ToolStripMenuItem viewE = new ToolStripMenuItem("View Employees");
            ...
            void ViewE_Click(object? sender, EventArgs e)
            {
                string showstr = "";
                if (cur_node.employee_nodes.Count() == 0)
                {
                    showstr = "There are no employees associated with this node.";
                }
                else
                {
                    int showli = 1;
                    foreach(Node enode in cur_node.employee_nodes)
                    {
                        showstr += showli.ToString()+". "+enode.name+"\n";
                        showli++;
                    }
                }
            ...
            void EditItem_Click(object? sender, EventArgs e)
            {
                ...
                roleForm.roleform_proj_lead.Checked = cur_node.proj_lead;
                roleForm.roleform_confirm.Click += roleform_click;
                if (cur_node.childnodes.Count > 0)
                {
                    foreach (Node enode in cur_node.childnodes)
                    {
                        if (enode.childnodes.Count > 0 || enode.proj_lead)
                        {
                ...
                if (cur_node.last_leaf) { roleForm.roleform_proj_lead.Enabled = false; }
                ...
                void roleform_click(object? sender, EventArgs e)
                {
                    try
                    {
                        ...
                        if (roleForm.roleform_proj_lead.Checked)
                        {
                            foreach(Node childnode in cur_node.childnodes) 
                            { 
                                childnode.last_leaf =true;
                                foreach (Node ecnode in childnode.employee_nodes)
                                {
                                    ecnode.last_leaf = true;
                                }
                            }
                        }
                        else
                        {
                            foreach (Node childnode in cur_node.childnodes) 
                            { 
                                childnode.last_leaf = false; 
                                foreach (Node ecnode in childnode.employee_nodes)
                                {
                                    ecnode.last_leaf = false;
                                }
                            }
                        }
                        Tree ETree = new Tree(e_root_node);
                        foreach (Node enode in cur_node.employee_nodes)
                        {
                            Debug.WriteLine(enode.name);
                            enode.role = cur_node.role;
                            enode.proj_lead = cur_node.proj_lead;
                            enode.last_leaf = cur_node.last_leaf;
                            enode.display_name = enode.name + " - " + cur_node.role + " (S$" + enode.salary.ToString() + ")";
                            List<Node> check_name = ETree.namesrecursive(enode);
                            if (check_name.Count > 1)
                            {
                                foreach (Node inode in check_name)
                                {
                                    if (inode == enode)
                                    {
                                        check_name.Remove(inode);
                                        check_name.Insert(0, inode);
                                        break;
                                    }
                                }

                                check_name[0].display_name = check_name[0].name + " - " + check_name[0].role + " (S$" + check_name[0].salary.ToString() + ")" + $" (Also {check_name[1].role})";
                                check_name[1].display_name = check_name[1].name + " - " + check_name[1].role + " (S$" + check_name[1].salary.ToString() + ")" + $" (Also {check_name[0].role})";
                            }
                        }
                        updateTree.Invoke(this, new EventArgs());
                        roleForm.Close();
        ...
        public void for_employee_tree_view(TreeView treeView2, Form2 form2)
        {
            ...
            void for_each_node_clicked(object sender, TreeNodeMouseClickEventArgs e)
            {
                ...
                if (e.Button == MouseButtons.Right)
                {
                    ...
                    if (cur_node.projects!=""&& cur_node.proj_lead)
                    {
                        adde.Enabled = false;
                    }
                }
                ...
                if(!(cur_node.got_all_roles))
                {
                    ...
                    void Eform_confirm_Click(object? sender, EventArgs e)
                    {
                        try
                        {
                            ...
                            employee.create_employee(cur_node.name, eform.eform_rofficer.Text, cur_node.salary, eform.eform_role.Text, "", cur_node.dummy_data, cur_node.salary_accountable, (eform.eform_role.SelectedItem as getNode).node);
            ...
            void Removee_Click(object? sender, EventArgs e)
            {
                Tree BTree = new Tree(root_node);
                Tree ETree = new Tree(e_root_node);
                if (!(cur_node.childnodes.Count==0 && cur_node.projects == "")) 
                {
            ...
            bool calc_new_proj_rev(int new_s, string proj_name, int past_s = 0)
            {
                ...
                {
                    if (x.projects!=null&&x.projects.Contains(proj_name)&&x.name!="Root")
                    {
                        total += x.salary;
            ...
            void Adde_Click(object? sender, EventArgs e)
            {
                ...
                void eform_click(object? sender, EventArgs e)
                {
                    try
                    {
                        ...
                        if (eform.eform_name.Text == "" || int.Parse(eform.eform_salary.Text)<1 || eform.eform_role.Text=="" || (int.Parse(eform.eform_salary.Text)>=cur_node.salary && cur_node!=e_root_node)) { throw new Exception(); }
                        if (cur_node.projects != "" && cur_node.proj_lead) 
                        {
                        ...
                        Node employee = new Node();
                        employee.create_employee(eform.eform_name.Text, cur_node.name, int.Parse(eform.eform_salary.Text), eform.eform_role.Text, "", eform.eform_dummy_data.Checked, eform.eform_salary_acc.Checked, (eform.eform_role.SelectedItem as getNode).node);
                        ETree.addNode(cur_node, employee);
                        if (cur_node.projects !="")
                        {
                            employee.projects = cur_node.projects;
                        }
                        updateTree.Invoke(this, new EventArgs());
                        eform.Close();
                    }catch (Exception ex)
        ...
        public void loadfileio()
        {
            ...
            {
                Node newnode = new Node();
                newnode.create_employee("Root", null, 0, "Root", "", false, true, root_node);
                e_root_node = newnode;
            }
            updateTree.Invoke(this, new EventArgs());
        }
        ...
        public void for_projects(Form3 form3)
        {
            ...
            void confirm_clicked(object? sender, EventArgs e, TextBox projname, TextBox revenue, ComboBox tlead, bool isedit=false)
            {
                try
                {
                    ...
                    if (rev_val == 0) { rev_val = tlead_node.proj_revenue; }
                    ...
                    tlead_node.proj_revenue = int.Parse(revenue.Text);
            ...
            void search_button_clicked(object? sender, EventArgs e, ComboBox comboBox, TextBox textbox, bool isedit)
            {
                ...
                try
                {
                    ...
                    if ((comboBox.SelectedItem != null && isedit)|| changenode != null)
                    {
                        if(comboBox.SelectedItem is getNode)
                        {
            ...
            void search_for_other_proj_leads(List<getNode> proj_leads, Node x, int max)
            {
                ...
                foreach (Node childNode in x.childnodes)
                {
                    if (childNode.proj_lead && (childNode.projects == ""||childNode.projects==null))
                    {
                        bool full_team= ETree.check_for_full_team(childNode);
                        if (!full_team) { break; }
                        int total = childNode.salary;
